...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        query_dim += freq_embedding_dim + time_embedding_dim
        ...
        self.to_q = LoRACompatibleLinear(query_dim, query_dim, bias=bias)

        if not self.only_cross_attention:
            # only relevant for the `AddedKVProcessor` classes
            self.to_k = LoRACompatibleLinear(self.cross_attention_dim, self.cross_attention_dim, bias=bias)
...
# v6
@torch.no_grad()
def get_embeddings(hidden_states_shape, freq_embedding_dim, time_embedding_dim, dtype=torch.float32, device="cuda"): # these embeddings are an extremely refined form of withcraft
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        # linear proj
        v_hidden_states = attn.to_out[0](v_hidden_states, scale=scale)
        # dropout
        v_hidden_states = attn.to_out[1](v_hidden_states)
