...
class UNetTrainer(ModuleTrainer):
    ...
    def __init__(self, config: UNetTrainerConfig, trainer: DualDiffusionTrainer) -> None:
        ...
        if self.config.num_loss_buckets > 0:
            ...
            self.bucket_names = [f"unet_loss_buckets/b{i} s:{bucket_ln_sigma[i]:.3f} ~ {bucket_ln_sigma[i+1]:.3f}" for i in range(self.config.num_loss_buckets)]
    ...
    def init_batch(self) -> None:
        ...
        if self.config.sigma_distribution == "ln_pdf":
            ...
            ln_sigma_error = self.module.logvar_linear(self.module.logvar_fourier(ln_sigma/4)).float().flatten().detach()
    ...
    def train_batch(self, batch: dict, grad_accum_steps: int) -> dict:
        ...
        if self.trainer.config.dataloader.use_pre_encoded_latents:
            ...
            samples = self.trainer.vae.encode(samples.to(self.trainer.vae.dtype), vae_class_embeddings, self.trainer.pipeline.format).mode().detach()
        ...
        samples = samples * self.module.sigma_data
        ...
        batch_loss_weight = (batch_sigma ** 2 + self.module.sigma_data ** 2) / (batch_sigma * self.module.sigma_data) ** 2
        ...
        if self.config.num_loss_buckets > 0:
            ...
            global_sigma_quantiles = self.trainer.accelerator.gather(self.module.sigma_data / batch_sigma.detach().cpu()).arctan() / (torch.pi/2)
