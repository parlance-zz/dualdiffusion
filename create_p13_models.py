...
unet_cfg=None
dae = DAE(DAE_Config(model_channels=64, unet=unet_cfg))
...
from modules.phaseret.phaseret_q432 import PhaseRet, PhaseRetConfig
phaseret = PhaseRet(PhaseRetConfig())
print_module_info(phaseret, "phaseret")

if input("Save module? (y/n) ").lower() == 'y':
    phaseret.save_pretrained(model_path, subfolder="phaseret")
    print(f"Saved model to {model_path}/phaseret")
